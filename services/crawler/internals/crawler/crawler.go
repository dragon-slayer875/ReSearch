...
func (worker *Worker) getNextDomain() (string, error) {
	domainWithScore, err := worker.redisClient.BZPopMin(worker.crawlerCtx, 0, redis.DomainPendingQueue).Result()
	if err != nil {
		return "", err
	}
...
func (worker *Worker) getNextUrlForDomain(domain string) (string, error) {
	url, err := worker.redisClient.RPop(worker.workerCtx, redis.CrawlQueuePrefix+domain).Result()
	if err != nil {
		return "", err
	}
